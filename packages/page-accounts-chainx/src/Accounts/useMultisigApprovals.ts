...
export default function useMultisigApprovals (address: string): [H256, Multisig][] {
  ...
  useEffect((): void => {
    events
      .filter(
        ({ record: { event: { section } } }) => section === 'multisig'
      )
      .reduce(
        (
          hasMultisig: boolean,
          ...
          data.reduce(
            (hasMultisig: boolean, item) => hasMultisig || (item.toRawType() === 'AccountId' && item.eq(address)),
            hasMultisig
          ),
        false
      ) && incTrigger();
    ...
    multiModule &&
      ...
      multiModule.multisigs
        ...
        .then((infos: [StorageKey, Option<Multisig>][]): void => {
          mountedRef.current &&
            setMultiInfos(
              infos.filter(([, opt]) => opt.isSome).map(([key, opt]) => [key.args[1] as H256, opt.unwrap()])
            );
        })
        .catch(console.error);
